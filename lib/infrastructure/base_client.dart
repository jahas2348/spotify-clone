...
import 'package:spotify_clone/infrastructure/access_token.dart';
import 'package:spotify_clone/models/podcast_info.dart';
...
class SpotifyApiService {
  ...
  // Generate a random seed to get random results with each request
  final String randomSeed = DateTime.now().millisecondsSinceEpoch.toString();

  // Use the 'album' type to search for albums
  // final String apiUrl = 'https://api.spotify.com/v1/search?q=album&type=album&limit=10&seed=$randomSeed';
  ...
  if (response.statusCode == 200) {
    ...
    if (jsonResponse.containsKey('albums')) {
      ...
      albums.shuffle();
      return albums.map((json) => SPotifyPodcast.fromJsonForPodcast(json)).toList();
    } else {
  ...
  static Future<List<SpotifyTrack>> fetchTrackItems(String apiUrl) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonResponse.containsKey('playlists')) {
        ...
        return playlists.map((json) => SpotifyTrack.fromJsonForPlaylist(json)).toList();
      } else if (jsonResponse.containsKey('tracks')) {
        final List<dynamic> tracks = jsonResponse['tracks'];
        return tracks.map((json) => SpotifyTrack.fromJsonForTrack(json)).toList();
      }
      else {
        throw Exception('Invalid response format');
      }
